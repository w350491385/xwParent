CREATE TABLE `book` (
  ...
  `infoDeal` tinyint(4) NOT NULL DEFAULT '0' COMMENT '1:关闭信息;2:删除信息',
  `auditTime` timestamp NULL DEFAULT NULL COMMENT '审核时间',
  `merchantMobile` tinyint(4) NOT NULL DEFAULT '0' COMMENT '商家账号处理 0：不处理 1：处理',
  `publishMobile` tinyint(4) NOT NULL DEFAULT '0' COMMENT '发布者账号处理 0：不处理 1：处理',
  PRIMARY KEY (`id`),
